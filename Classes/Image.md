...
## 方法
### (static) fromElement(element, optionsopt, callback) → {fabric.Image}
    静态方法
    从SVG元素返回fabric.Image实例
参数|类型|性质|描述
|---|---|---|---
element|SVGElement|必填|要解析的元素
options|Object|选填|选项
callback|function|必填|解析完成后调用的选项回调
返回：`fabric.Image`，fabric.Image的实例
...
参数|类型|性质|描述
|---|---|---|---
filtered|Boolean|必填|指示svg是否需要src
返回：`String`，图片来源
