...
link：http://fabricjs.com/docs/fabric.Triangle.html
...
### type
    对象的类型（矩形，圆形，路径等）。
    请注意，此属性是只读的，而不是要修改的。如果您进行修改，Fabric的某些部分（例如JSON加载）将无法正常工作。
赋值类型：`String(triangle)`
